...
def plot_dataset(values: np.ndarray, title: str = None, subplot: list = None):
    """Plot the dataset.
    ...
    fig, ax = plt.subplots()
    ax2 = ax.twinx()
    ax.plot(values['t'], values['V'], color='b', label='Voltage')
    ax2.plot(values['t'], values['I'], color='r', label='Current')
